...
FixSCPFLS::FixSCPFLS(LAMMPS *lmp, int narg, char **arg) : FixSCPF(lmp,narg,arg)
{
  searchflag = 1;
  gextra = hextra = NULL;
...
void FixSCPFLS::init()
{
  ...
  if (linestyle == 0) linemin = &FixSCPFLS::linemin_backtrack;
  else if (linestyle == 1) linemin = &FixSCPFLS::linemin_quadratic;
  else if (linestyle == 2) linemin = &FixSCPFLS::linemin_forcezero;
...
void FixSCPFLS::reset_vectors()
{
  ...
  if (nextra_atom) {
    ...
    for (int m = 0; m < nextra_atom; m++) {
      ...
      x0extra_atom[m] = request_vector(n++);
      gextra_atom[m] = request_vector(n++);
      hextra_atom[m] = request_vector(n++);
    }
  }
}
...
int FixSCPFLS::linemin_backtrack(double eoriginal, double &alpha)
{
  int i,m,n;
...
int FixSCPFLS::linemin_quadratic(double eoriginal, double &alpha)
{
  int i,m,n;
  double fdothall,fdothme,hme,hmax,hmaxall;
  double de_ideal,de;
...
int FixSCPFLS::linemin_forcezero(double eoriginal, double &alpha)
{
  int i,m,n;
  double fdothall,fdothme,hme,hmax,hmaxall;
  double de;
...
double FixSCPFLS::alpha_step(double alpha, int resetflag)
{
  int i,n,m;
...
double FixSCPFLS::compute_dir_deriv(double &ff)
{
   int i,m,n;
   double *hatom, *fatom;
   double dot[2],dotall[2];
