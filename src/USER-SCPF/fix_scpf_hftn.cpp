...
static const int  STOP_MAX_ITER = FixSCPF::MAXITER;        //-- MAX ITERATIONS EXCEEDED
static const int  STOP_MAX_FORCE_EVALS = FixSCPF::MAXEVAL; //-- MAX FORCE EVALUATIONS EXCEEDED
static const int  STOP_ENERGY_TOL = FixSCPF::ETOL;         //-- STEP DID NOT CHANGE ENERGY
static const int  STOP_FORCE_TOL = FixSCPF::FTOL;          //-- CONVERGED TO DESIRED FORCE TOL
static const int  STOP_TR_TOO_SMALL = FixSCPF::TRSMALL;    //-- TRUST REGION TOO SMALL
static const int  STOP_ERROR = FixSCPF::INTERROR;          //-- INTERNAL ERROR
...
FixSCPFHFTN::FixSCPFHFTN(LAMMPS *lmp, int narg, char **arg) : FixSCPF(lmp, narg, arg)
{
  searchflag = 1;
...
void FixSCPFHFTN::reset_vectors()
{
  nvec = 3 * atom->nlocal;
  ...
  for (int  i = 0; i < NUM_HFTN_ATOM_BASED_VECTORS; i++)
    _daAVectors[i] = request_vector (i);
  ...
  if (nextra_atom) {
    ...
    for (int m = 0; m < nextra_atom; m++) {
      ...
      for (int  i = 0; i < NUM_HFTN_ATOM_BASED_VECTORS; i++)
        _daExtraAtom[i][m] = request_vector (n++);
    }
  }
...
------------------------------------------------------------------------- */
int FixSCPFHFTN::execute_hftn_(const bool      bPrintProgress,
                           ...
                           double &  dFinalForce2)
{
  ...
  for (int nit = 0; nit < maxiter; nit++) {
    ...
    if (timer->check_timeout(nit))
      return(FixSCPF::TIMEOUT);

    niter++;

    //---- CALL THE INNER LOOP TO GET THE NEXT TRUST REGION STEP.

    double  dCgForce2StopTol = MIN ((dCurrentForce2 / 2.0), 0.1 / (nit+1));
    dCgForce2StopTol = MAX (dCgForce2StopTol, ftol);
    ...
    if (nStepType == NO_CGSTEP_BECAUSE_F_TOL_SATISFIED) {
      if (bPrintProgress)
        hftn_print_line_ (true, nit+1, neval, dNewEnergy, dNewForce2,
    ...
    if (nStepType != CGSTEP_TO_DMAX) {
      ...
      if (   (fabs (dAred) < (etol * dMag))
             || (dStepLengthInf == 0.0) ) {
        if (bPrintProgress)
          hftn_print_line_ (true, nit+1, neval,
                            dNewEnergy, dNewForce2,
                            nStepType, dTrustRadius, dStepLength2,
                            dAred, -1.0);
        //---- (IMPLICITLY ACCEPT THE LAST STEP TO THE NEW POINT.)
    ...
    if (bStepAccepted) {
      ...
      if (bPrintProgress)
        hftn_print_line_ (false, nit+1, neval,
                          dCurrentEnergy, dCurrentForce2,
                          nStepType, dTrustRadius, dStepLength2,
                          dAred, dPred);
...
bool FixSCPFHFTN::compute_inner_cg_step_(const double    dTrustRadius,
...
double FixSCPFHFTN::calc_xinf_using_mpi_(void) const
{
...
double FixSCPFHFTN::calc_dot_prod_using_mpi_(const int  nIx1,
...
double FixSCPFHFTN::calc_grad_dot_v_using_mpi_(const int  nIx) const
{
...
void FixSCPFHFTN::calc_dhd_dd_using_mpi_(double &  dDHD,
                                     double &  dDD) const
{
...
void FixSCPFHFTN::calc_ppnew_pdold_using_mpi_(double &  dPnewDotPnew,
...
void FixSCPFHFTN::calc_plengths_using_mpi_(double &  dStepLength2,
...
bool FixSCPFHFTN::step_exceeds_TR_(const double    dTrustRadius,
                               const double    dPP,
                               const double    dPD,
                               const double    dDD,
                               double &  dTau) const
...
bool FixSCPFHFTN::step_exceeds_DMAX_(void) const
{
...
void FixSCPFHFTN::adjust_step_to_tau_(const double tau)
{
  if (nextra_global) {
    double *  pGlobal  = _daExtraGlobal[VEC_CG_P];
    double *  dGlobal  = _daExtraGlobal[VEC_CG_D];
...
double FixSCPFHFTN::compute_to_tr_(const double  dPP,
                               const double  dPD,
                               const double  dDD,
...
void FixSCPFHFTN::evaluate_dir_der_(const bool      bUseForwardDiffs,
                                const int       nIxDir,
                                const int       nIxResult,
...
void FixSCPFHFTN::open_hftn_print_file_(void)
{
  int  nMyRank;
...
void FixSCPFHFTN::hftn_print_line_(const bool    bIsStepAccepted,
...
void FixSCPFHFTN::close_hftn_print_file_(void)
{
