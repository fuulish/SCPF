...
FixSCPFCG::FixSCPFCG(LAMMPS *lmp, int narg, char **arg) : FixSCPFLS(lmp, narg, arg) {}
FixSCPFCG::~FixSCPFCG() {}
...
int FixSCPFCG::iterate(int maxiter)
{
  ...
  for (int iter = 0; iter < maxiter; iter++) {
    ...
    // output for thermo, dump, restart files, FUDO| is not done

    // if (output->next == ntimestep) {
    //   timer->stamp();
    //   output->write(output->next_dump_any);
